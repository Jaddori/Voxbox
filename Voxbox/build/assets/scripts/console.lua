...
function console:load()
	...
	self.input.commands = {}
	for i=1, CONSOLE_INPUT_MAX_COMMANDS do
		self.input.commands[i] = ""
	end
end
	...
	if self.visible then
		...
		if Input.keyRepeated( Keys.Up ) then
			...
			if self.input.currentCommand <= 0 then
				self.input.currentCommand = CONSOLE_INPUT_MAX_COMMANDS
			end
		...
		if Input.keyRepeated( Keys.Down ) then
			self.input.currentCommand = self.input.currentCommand + 1
			if self.input.currentCommand > CONSOLE_INPUT_MAX_COMMANDS then
